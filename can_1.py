...
def read_csv(filename, delimiter, **kwargs):
	if "dtype" in kwargs and "chunksize" in kwargs:
		...
		return pd.concat(pd.read_csv(filename, delimiter = delimiter, dtype = kwargs["dtype"]))
	elif "chunksize" in kwargs:
		return pd.concat(pd.read_csv(filename, delimiter = delimiter, chunksize = kwargs["chunksize"]))
	else:
		return pd.concat(pd.read_csv(filename, delimiter = delimiter))
...
basedf = read_csv("shortsales.csv", ",", chunksize = 10000)
basedf.sort_values("product_id", ascending = "True", inplace = True)
basedf.reset_index(drop= True, inplace = True)

hierdf = read_csv("product_hierarchy.csv", "|", dtype = "int8")
citydf = read_csv("store_cities.csv", "|")
...
basedf["city_id"] = np.asarray(hiers.iloc[basedf["product_id"]-1]["city_id"])
